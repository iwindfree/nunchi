...
**결론부터 말하면 일부는 정당하고 일부는 설계를 피한 것입니다.** 둘을 구분할
...
Rust가 `Clone`을 자동으로 하지 않는 이유가 있습니다. **비용이 드는 일을 몰래
하지 않기 때문입니다.** `.clone()`이 코드에 보이면 거기서 힙 할당과 복사가
일어난다는 사실이 드러납니다.
...
`Clone`은 자동이 아니라 직접 불러야 합니다. Rust가 비용이 드는 일을 몰래
하지 않기 때문입니다.
