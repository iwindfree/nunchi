...
134번 나오고, 이터레이터 메서드를 합치면 581번입니다. **이 코드의 골격입니다.**
...
계산이 일어나지 않고, 마지막에 거두는 단계에서야 한꺼번에 돕니다.
...
이터레이터는 값을 하나씩 꺼내 주며, 만들고 변형하고 거두는 세 단계로
이루어집니다. 변형 단계는 실행되지 않고 거두는 단계에서 한꺼번에 돕니다.
