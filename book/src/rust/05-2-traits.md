...
**이 구분이 실제로 도움이 됩니다.** 다른 데이터베이스로 옮길 때, 트레이트에 있는
...
트레이트는 **여러 타입이 같은 약속을 지켜야 할 때** 유용합니다. 그런 상황이
많지 않으면 굳이 만들 이유가 없습니다.

nunchi에서 그런 상황은 저장 계층 하나뿐이었습니다. 추출기는 언어마다
다르지만 `SupportedLang` 열거형으로 구분하면 충분했고, 프레임워크 규칙은
...
여섯 개로 제한하는 것이 그 장치가 동작하는 조건입니다.
