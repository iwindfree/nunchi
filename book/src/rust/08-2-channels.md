...
클로저는 우리 스레드가 아니라 라이브러리가 만든 스레드에서 불립니다.**
...
돌아 버리고, 여러 스레드가 동시에 데이터베이스를 건드리게 됩니다.
...
**첫째, `move`가 필수입니다.** 클로저가 다른 스레드에서 불리므로 `tx`를
...
- `Timeout`은 200밀리초 동안 아무 일도 없었습니다. 정상이므로 아무것도 하지
  않습니다.
- `Disconnected`는 보내는 쪽이 전부 사라졌습니다. 워처가 죽었다는 뜻이므로
  반복을 끝냅니다.
...
**사건이 올 때마다 시계를 다시 시작합니다.** 변경이 계속되는 동안에는
...
다른 스레드에서 불리는 클로저는 `move`로 소유권을 가져가야 합니다.
