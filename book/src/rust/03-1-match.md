...
`match`는 값이 어떤 경우인지 따져 갈라지는 문법입니다. nunchi 코드에 57번
나옵니다.
...
이것을 빠짐없음 검사(exhaustiveness check)라고 부릅니다. **열거형에 값을
추가하면 그것을 다루지 않은 모든 `match`가 컴파일 오류가 됩니다.** 고쳐야 할
곳을 컴파일러가 전부 찾아 줍니다.

nunchi에서 실제로 값을 했습니다. 엣지 종류를 19개까지 늘리는 동안, 새 종류를
...
값을 추가해도 컴파일러가 알려 주지 않기 때문입니다. 빠짐없음 검사의 이득이
사라집니다.
...
tree-sitter가 알려 준 노드 종류에 따라 갈라집니다. `fetch("/api/x")`와
...
가드를 써서 후보 개수에 따라 갈라지는 함수를 작성하는 문제입니다.
