...
그대로 담았기 때문입니다. `cargo run -p nunchi-desktop` 한 줄이면 빌드하고
실행할 수 있습니다.

`tauri-cli`를 따로 설치할 필요도 없습니다. 빌드에 필요한 `tauri-build`가
빌드 의존성으로 들어 있기 때문입니다. `tauri-cli`는 `.app`이나 `.msi` 같은
설치 파일을 만들 때 쓰는데, 지금은 `bundle.active`를 꺼 두었으므로 그 단계가
없습니다.
