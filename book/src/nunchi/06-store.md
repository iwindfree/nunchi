...
`Store`는 인터페이스이고 `SqliteStore`는 구현입니다. 사이에 트레이트를 둔 이유도
뒤에서 설명합니다.
...
메서드가 여섯 개뿐입니다. **이 개수를 적게 유지하는 것이 의도된 설계입니다.**

트레이트가 좁으면 다른 저장소로 바꿀 때 구현할 것이 적습니다. 실측으로는
하루 안에 교체할 수 있는 수준입니다. 메서드가 스무 개였다면 교체가 사실상
불가능해집니다.

[5.2장](../rust/05-2-traits.md)에서 다룬 트레이트가 여기서 실제로
쓰입니다. 다만 이 코드베이스에 트레이트 정의는 이것 **하나뿐**입니다.
...
`key`가 따로 있는 이유는 Windows 때문입니다. NTFS는 대소문자를 구분하지
...
`"delete Comment"`로도 넣어 두면 자연어 질의가 카멜케이스 식별자와 연결됩니다.
...
엣지가 없는 루트 노드입니다. "참조가 없으면 삭제"를 그대로 적용하면 저장소
노드부터 사라집니다.
...
그리고 앞서 말한 쓰기 프로세스가 하나뿐이어야 한다는 제약이 있습니다.
...
다른 저장소로 바꿀 때 이 부분은 따로 대응해야 합니다. 트레이트를 좁게
