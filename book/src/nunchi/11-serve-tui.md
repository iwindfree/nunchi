...
`match`가 도구 이름에 따라 분기합니다. 마지막 `other =>`가 없으면 컴파일되지
않습니다. 문자열은 경우의 수가 무한하므로 컴파일러가 모든 경우를 다뤘음을 보장할 수
없기 때문입니다([3.1장](../rust/03-1-match.md)).

### 인덱스가 낡았으면 답에 붙여 보냅니다

에이전트는 **자기가 받지 못한 것을 알 수 없습니다.** 인덱스가 낡으면 팩은
낡은 항목을 조용히 버리고 새 파일은 아예 나오지 않는데, 받는 쪽에서는 그냥
결과가 좀 적을 뿐입니다.

그래서 모든 답에 인덱스 상태를 함께 실어 보냅니다.
...
어긋나지 않았으면 아무것도 붙지 않습니다. 이상이 없을 때 조용한 것이
중요합니다. 매번 "정상입니다"를 붙이면 그것도 토큰이고, 곧 읽지 않게 됩니다.

무엇을 해야 하는지까지 적습니다. 서버 안내문에도 넣었습니다.

> 응답에 stale_index 가 있으면 인덱스가 실제 코드와 어긋난 것입니다. 결과에
> 빠진 것이 있을 수 있으니 그 사실을 사용자에게 알리고 `nunchi index` 실행을
> 권하세요.

재는 간격은 60초입니다. 도구를 부를 때마다 저장소를 훑으면 큰 저장소에서
질의보다 검사가 비싸집니다. 그렇다고 띄울 때 한 번만 재면 긴 세션 도중에
`git pull`을 한 경우를 놓칩니다. 자세한 것은 [10장](10-watch.md)에 있습니다.
...
`stdio()`는 표준 입출력으로 메시지를 주고받습니다. **표준 출력은 JSON-RPC 메시지 전용입니다.**
...
에이전트가 잘못된 결과를 냈을 때 원인이 **추출 실패인지 랭킹 오류인지 인덱스가
오래된 것인지** 사람이 구분할 수단이 필요합니다. 그것이 TUI의 존재 이유입니다.
...
`match key.code`에 조건이 붙은 분기가 있습니다.
...
MCP 서버와 TUI는 같은 `nunchi-core`를 부르는 두 인터페이스입니다. 그래서 TUI에
보이는 것과 에이전트가 받는 것이 일치합니다.

MCP 도구를 다섯 개로 제한한 것은 스키마가 상시 비용이기 때문입니다. 여러 질의를
묶어 실행할 때는 CLI를 씁니다.

인덱스가 낡았으면 그 사실을 답에 실어 보냅니다. 에이전트는 자기가 받지 못한
것을 알 수 없기 때문입니다.

TUI는 조회 전용입니다. 고유 기능을 두면 자동화가 막힙니다.

다음 장에서 같은 일을 창으로 하는 데스크톱 앱을 봅니다. 터미널을 쓰지 않고
설정을 만들고 인덱싱까지 하는 화면입니다.
