...
└── nunchi-desktop/  창을 띄우는 앱입니다. 화면만 담당합니다
```

로직을 전부 라이브러리에 둔 이유가 있습니다. CLI와 MCP 서버와 TUI와 데스크톱
앱이 **같은 코드를 부르게** 하기 위해서입니다. 넷이 각자 구현하면 결과가
달라지고, 화면에 보이는 것과 에이전트가 받는 것이 어긋나게 됩니다.
...
| `core/init.rs` | 설정 파일을 처음 만듭니다 | [12장](12-desktop.md) |
...
| `desktop/main.rs` | 웹뷰가 부를 커맨드를 정의합니다 | [12장](12-desktop.md) |
| `desktop/query.rs` | 탐색과 팩 화면의 조회를 맡습니다 | [12장](12-desktop.md) |
| `desktop/settings.rs` | 설정을 폼과 원문으로 고칩니다 | [12장](12-desktop.md) |
...
MCP 서버(`serve.rs`)와 데스크톱 앱(`desktop/query.rs`)도 같은 `pack.rs`를
부릅니다. 인터페이스만 다르고 하는 일은 같습니다.
