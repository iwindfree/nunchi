...
이 장에서 다루는 코드가 이 프로그램의 핵심 목적을 수행합니다. 지금까지 만든 그래프로 **토큰 절감**을
실제로 만들어 내는 부분입니다.
...
`expand_query`가 한국어 질의가 영어 식별자와 연결되게 만듭니다. 설정에
...
그래서 원인과 해결책을 함께 알려 줍니다. 한국어 사용자가 가장 먼저 겪는
문제입니다.
...
일치가 있거나 그래프 근접도가 임계값을 넘어야 후보가 됩니다.**

이 임계값이 없었을 때 문제가 생겼습니다. `save`나 `getBySlug` 같은 연결이 많은
...
파일 점수를 낮게 준 이유가 있습니다. **파일은 심볼을 담는 단위일 뿐입니다.**
...
`if *score < floor { break; }`가 예산을 판단하는 부분입니다.
...
L1은 15줄, L2는 400줄까지 담습니다. 이 강등이 60k 토큰을 4k 토큰으로 줄이는
핵심 수단 중 하나입니다.
...
확신을 담아 주는 것보다 오래되었다고 알리는 편이 언제나 낫습니다.**
...
지표가 만족스러워질 때까지 계속 조정하는 것은 위험합니다. 그래프가 grep보다 나은 결과를 내지
...
관련성 임계값이 없으면 연결이 많은 심볼이 중심성만으로 들어옵니다. 예산 하한이
없으면 관련 코드가 적은 작업에서 오히려 손해가 납니다. 둘 다 벤치가
드러냈습니다.

실제 코드와 어긋난 인덱스는 `stale`로 표시합니다. 잘못된 좌표를 주는 것보다 낫습니다.
