...
| SCIP 정밀 경로 | 착수하지 않았으며 남은 조각 중 가장 큽니다 |
...
모든 로직은 `nunchi-core`에 있고 CLI는 얇은 진입점입니다. MCP 서버와 CLI,
...
```
파일 워크 (ignore 크레이트를 쓰며 디렉터리 단위로 가지를 칩니다)
   │
...
모든 엣지에 출처(`fast` 또는 `precise`)와 확신도를 붙입니다. 추정으로 만든
...
출력은 JSON-RPC 전용 통로이므로, 로그가 그곳에 섞이면 프로토콜이 깨집니다.
실제로 겪은 문제입니다.
...
| 참조 해소 | `resolve.rs` | 후보 수에 따른 확신도 분기 |
...
프런트 API 호출 4 — 백엔드 라우트 연결 4 (100%) · 동적 1건 제외
```
...
### 1. 교차 저장소 연결률 95%는 오탐이 만든 착시였습니다
...
때문에 백엔드와 잘 연결되었고, 그 결과 지표가 부풀려졌습니다.
...
확신도 0.9)을 주 경로로 삼고, 호출에 기반한 판정은 메서드와 함수, 생성자,
클래스로 제한했습니다(확신도 0.6). 그 결과 628건에서 156건으로 줄었고 남은
연결은 모두 의미가 있었습니다.

### 튜닝을 멈춘 지점

벤치의 "사용자 인증 로그인" 작업은 절감량이 -14%로 남아 있습니다. 그래프가
grep을 이기지 못합니다. 원인을 확인했는데, 그 저장소의 인증 코드가 작은 파일
몇 개에 모여 있어서 grep이 정확히 찾아냅니다. 팩에 담긴 항목은 모두 실제로
인증과 관련된 코드였습니다.

**여기서 튜닝을 멈췄습니다.** 숫자가 좋아 보일 때까지 계속 조정하는 것은 위의
세 사례와 같은 잘못입니다. 그래프가 이기지 못하는 경우를 드러내는 것도 벤치가
하는 일입니다.
...
2. **실제 솔루션에 적용하고 벤치를 실행하는 일**입니다. 업무 작업으로 다시
   ...
   가장 값진 자산이 됩니다.
3. **SCIP 정밀 경로 연동**입니다(`scip-java`, `scip-typescript`). 남은 조각 중
   가장 큽니다. 현재는 이름에 기반한 추정만 있습니다. 빌드가 필요하므로 커밋과
   CI, 유휴 시간에만 실행하는 2단 구조로 설계해 두었습니다. `Provenance`의
   `Fast`와 `Precise` 구분이 이미 자리를 잡고 있습니다.
...
6. **WinForms 심화**입니다. Designer 파일의 이벤트 배선(`btnSave.Click += ...`)을
   `HANDLES` 엣지로 만들어야 합니다. `partial` 병합은 이미 완료했습니다.
7. **파일 단위 증분 갱신**입니다. 워처가 아직 전체를 다시 인덱싱합니다. 캐시
   덕분에 비용은 낮습니다.
