...
| 신선도 검사 | 인덱스가 실제 코드와 어긋난 정도를 재어 MCP·doctor·앱에 알립니다 |
...
### HTTP 클라이언트 호출 형태를 추가할 때

`rules/builtin.<언어>.toml`의 `[[http_client]]`에 넣습니다. 무엇을 어디서
읽을지는 다음 네 가지로 적습니다.

| 필드 | 언제 쓰는가 |
|---|---|
| `method_option` | 설정 객체에 메서드가 있습니다. `fetch(url, { method: "POST" })` |
| `url_option` | 설정 객체에 URL이 있습니다. `axios({ method, url })` |
| `method_arg` | 메서드를 인자로 넘깁니다. `rest.exchange(url, HttpMethod.POST, ...)` |
| `method_from_receiver` | 이어 부릅니다. `webClient.get().uri("/api/x")` |

`framework.rs`의 `detects_the_shapes_real_code_uses`에 형태를 한 줄 더하면
됩니다. 그 테스트가 잡는 것과 넘기는 것을 함께 확인합니다.
...
6. **요청 빌더 지원**입니다. Spring `WebClient`의 `get().uri("/api/x")`는
   `method_from_receiver` 규칙으로 해결했습니다. 남은 것은 OkHttp처럼 빌더
   객체를 변수에 담아 여러 줄에 걸쳐 조립하는 형태입니다. 한 식 안에서
   이어 부르지 않으므로 호출 하나만 보아서는 읽을 수 없습니다.
7. **설정에서 주입받는 경로 지원**입니다. `@Value("${order.api.base}")`처럼
   값이 코드 밖에 있으면 소스만 봐서는 알 수 없습니다. 규칙 파일에 키와 값을
   등록하는 배열을 더하고, 인덱싱 2패스의 상수 표에 함께 넣으면 됩니다.
   자동 추출과 달리 사용자만 채우는 항목이므로 내장 기본값에는 빈 배열만
   둡니다.
8. **WinForms 심화**입니다. Designer 파일의 이벤트 배선(`btnSave.Click += ...`)을
   `HANDLES` 엣지로 만들어야 합니다. `partial` 병합은 이미 완료했습니다.
9. **파일 단위 증분 갱신**입니다. 워처가 아직 전체를 다시 인덱싱합니다. 캐시
   덕분에 비용은 낮습니다.
