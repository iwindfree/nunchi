...
max_candidates = 3          # 이름이 같은 심볼이 이보다 많으면 호출 해소를 포기합니다
```
...
**후보과다** 항목은 이름이 같은 심볼이 너무 많아 해소를 포기한 호출입니다.
`get`이나 `save`처럼 흔한 이름이 여기 해당합니다. 이 값이 크고 팩에서
관련 코드를 놓친다고 느껴지면 `nunchi.shared.toml`의 `index.max_candidates`를
올려 보십시오. 기본값은 3입니다.

값을 올리면 애매한 엣지가 늘어나는 대신 놓치는 호출이 줄어듭니다. 애매한
엣지는 신뢰도가 후보 수만큼 나뉘어 붙으므로 랭킹에서 확실한 엣지에
밀립니다. 다시 인덱싱한 뒤 `nunchi bench`로 재현율과 절감률을 함께
확인하십시오.
...
적고 이름이 뚜렷하면 grep이 정확히 찾아내므로 그래프가 더 나은 결과를 내지 못합니다.
...
한국어로 질의했을 때 영어 식별자와 연결되도록 만듭니다.
